    ...
    return (
        <div className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100">
            <div className="px-6 sm:px-10 py-6 sm:py-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
                <div>
                    <h2 className="text-2xl sm:text-3xl font-black text-slate-900">Worker Management</h2>
                    <p className="text-slate-400 font-bold mt-1 uppercase text-[10px] sm:text-xs tracking-widest">Managing Registered Service Providers</p>
                </div>
                <div className="bg-white px-4 sm:px-5 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 text-emerald-600 font-black text-lg sm:text-xl">
                    {workers.length} <span className="text-slate-400 text-xs sm:text-sm uppercase ml-1">Total</span>
                </div>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="text-left bg-slate-50 text-slate-400 uppercase text-[10px] sm:text-xs font-black tracking-widest">
                            <th className="px-6 sm:px-10 py-4 sm:py-6">Worker Profile</th>
                            <th className="px-6 sm:px-10 py-4 sm:py-6">Profession</th>
                            <th className="px-6 sm:px-10 py-4 sm:py-6">Region</th>
                             ...
                             <tr key={w._id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-6 sm:px-10 py-4 sm:py-6">
                                    <div className="flex items-center gap-3 sm:gap-4">
                                        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl overflow-hidden shadow-md border-2 border-white shrink-0">
                                            <img
                                        ...
                                        <div className="min-w-[120px]">
                                            <p className="font-black text-slate-800 text-base sm:text-lg leading-tight group-hover:text-emerald-700 transition-colors truncate">{w.name}</p>
                                            <p className="text-slate-400 font-bold text-xs sm:text-sm tracking-tight truncate">{w.email}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 sm:px-10 py-4 sm:py-6">
                                    <span className="bg-emerald-50 text-emerald-700 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest border border-emerald-100 whitespace-nowrap">
                                        {w.jobType}
                                    </span>
                                </td>
                                <td className="px-6 sm:px-10 py-4 sm:py-6">
                                    <div className="flex items-center gap-2 text-slate-500 font-bold text-xs sm:text-sm whitespace-nowrap">
                                        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                        {w.city}
                                    </div>
                                </td>
